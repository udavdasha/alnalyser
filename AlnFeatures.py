...
class AlnFeatures(tkinter.Frame):
    ...
    def create_UI(self):
        ...
        tkinter.Label(self.features.panel, text = "Max e-value:").grid(row = 0, column = 2, sticky = "NSW")
        ...
        tkinter.Label(self.features.panel, text = "Min overlap/max_domain_length (%):").grid(row = 0, column = 4, sticky = "NSW")
    ...
    def obtain_features(self):
        ...
        if self.host.verbose.get():
            os.system("%s -i %s -w %s -o %s %s -e %s -f %s -d %s" % (obtain_features_path, os.path.basename(fixed_filename), self.host.settings.work_dir, 
                                                               "%s.features" % self.host.temp_name, data_for_features, self.evalue_threshold.get(), self.overlap_threshold.get(), dom_filename))
        else:
            os.system("%s -i %s -w %s -o %s %s -e %s -f %s -d %s 1> nul 2> nul" % (obtain_features_path, os.path.basename(fixed_filename), self.host.settings.work_dir, 
                                                                             "%s.features" % self.host.temp_name, data_for_features, self.evalue_threshold.get(), self.overlap_threshold.get(), dom_filename))
        ...
        dom_filename_full = os.path.join(self.host.settings.work_dir, dom_filename)
        if os.path.isfile(dom_filename_full): # File was created (non-empty domains)
