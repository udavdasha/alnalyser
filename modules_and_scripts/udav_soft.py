...
def read_TMHMM_output(input_filename):
    """
...
def get_feature_from_Pfam(id_to_domains, add_score = False):
     ...
     Could add score information (e-value and score) to the feature string,
     ...
     for curr_id in id_to_domains.keys():
         ...
         for curr_name in id_to_domains[curr_id].keys():
             ...
             for f in fields:
                 (begin, end, e_value, score) = get_values(f)
                 if add_score:
                     feature_string += "%i..%i..%s..%s," % (begin, end, e_value, score)
                 else:
...
def remove_hit(remove_begin, remove_end, remove_name, protein_id, all_domains):
    ...
    for f in fields:
        (begin, end, evalue, score) = get_values(f)        
        if (begin == remove_begin) and (end == remove_end): # Removing
            continue
        all_domains[protein_id][remove_name] += "%s..%s..%s..%s " % (begin, end, evalue, score)
    ...
    if all_domains[protein_id][remove_name] == "": # This whole domain is marked for removal
        all_domains[protein_id][remove_name] = "0..0..100..0"          
...
def check_overlap(target_domain, target_domain_name, protein_id, domains, threshold):
    ...
    for tf in target_fields:                               # Itterating domain hits
        (target_begin, target_end, target_evalue, target_score) = get_values(tf)
        ...
        for domain_name in domains[protein_id].keys():
            ...
            for f in fields:                               # Itterating domain hits
                (curr_begin, curr_end, curr_evalue, curr_score) = get_values(f)
...
def unite_same_Pfam_hits(id_to_domains): #FIX: version 1.96
    """
    Currently the following parameters are fixed:
    <max_distance> = less than 50 aminoacids should be between domain pieces
    <max_overlap> = less overlap than this portion of the largest HMM parts (30%)
    """
    max_distance = 50
    max_overlap = 0.7
    n = 0
        ...
        for domain_name in id_to_domains[protein_id].keys():
            ...
            if len(curr_data) > 1: # More than a single hit of this domain found      
                ...
                while p < len(curr_data) - 1:
                     (fst_begin, fst_end, fst_evalue, fst_score) = get_values(curr_data[p])
                     (snd_begin, snd_end, snd_evalue, snd_score) = get_values(curr_data[p + 1])                            
                     try:
                         fst_hmm_begin = int(curr_data[p].split("..")[4])
                         fst_hmm_end = int(curr_data[p].split("..")[5])
                         snd_hmm_begin = int(curr_data[p + 1].split("..")[4])
                         snd_hmm_end = int(curr_data[p + 1].split("..")[5])
                         ...
                         if ((snd_begin - fst_end) < max_distance) and (curr_hmm_overlap < (max(fst_hmm_size, snd_hmm_size) * max_overlap)):
...
def read_Pfam_output(input_filename, max_e_value, filter_hits, threshold, add_score = False, unite_same = False):
    """
    ...
    If <unite_same> is True, hits of the same domain found (a) near each other and (b) with
    different parts of the HMM models will be united

    Results tuple of two values:
    (1) Hash of protein IDs to a string configured from domains found in a format like this:
    protein_id -> [domain name] s1..e1..evalue..(score),s2..e2..e..(s)\t[another domain] <etc>
    ...
    for string in input_file:
        ...
        if not protein_id in id_to_domains:
            ...
            if not domain_name in id_to_domains[protein_id]:
                id_to_domains[protein_id][domain_name] = ""
        id_to_domains[protein_id][domain_name] += "%s..%s..%s..%s..%s..%s " % (begin, end, evalue, score, hmm_begin, hmm_end)
    ...
    if unite_same == True:
        id_to_domains = unite_same_Pfam_hits(id_to_domains)
...
def read_plain_features(features_filename):
    """
...
def read_HMM_report(filename, reverse_order, type_of_id = "GI"):
    ...
    for string in report_file:
        ...
        if fields[0].count("COG") == 0: # No COG data
            if type_of_id == "GI":
...
#------------------------------------------------------------------------------
class COG_hit:
    def __init__(self, begin, end, COG):
    ...
    for f in files:        
        ...
        for string in single_file:
            ...
            if not gi in COG_assignment:
                COG_assignment[gi] = COG_hit(fields[4], fields[5], fields[6])    
                curr_uid = fields[1].split("_")[-1]
                ...
                if type(COG_assignment[gi]) != list: # This was not converted to list
                    existing_COG_hit = COG_hit(COG_assignment[gi].begin(), COG_assignment[gi].end(), COG_assignment[gi].COG_full)
                    COG_assignment[gi] = list()
                    COG_assignment[gi].append(existing_COG_hit)
                COG_assignment[gi].append(COG_hit(fields[4], fields[5], fields[6]))
                COG_assignment[gi].sort(cmp = sort_COGs)
            del fields
        n += 1
